...
```dockerfile
# Multi-stage build: Use gcc image for building the C++ application
FROM gcc:latest AS builder
 
# Set working directory for build stage
WORKDIR /usr/src/app
 
# Install build dependencies (cmake for build system, git for version control)
RUN apt-get update && apt-get install -y cmake git && rm -rf /var/lib/apt/lists/*
 
# Copy all source files to the container
COPY . .
 
# Clean any existing build artifacts, create build directory, and compile the C++ TCP server
RUN rm -rf build && mkdir build && cd build && cmake ../backend && make
 
# Runtime stage: Use fresh gcc image for running the compiled application  
FROM gcc:latest AS runtime
 
# Set working directory for runtime
WORKDIR /app
 
# Install Python3 (may be needed for runtime dependencies) and clean up
RUN apt-get update && apt-get install -y python3 && rm -rf /var/lib/apt/lists/*
 
# Copy the compiled server binary from builder stage
COPY --from=builder /usr/src/app/build/server ./server
 
# Copy data files needed by the TCP server (likely contains blacklist data)
COPY backend/data ./data
 
# Make the server binary executable
RUN chmod +x ./server
 
# Start the TCP server with default parameters (port 5555, 16 hash functions, 1 other param)
...
```dockerfile
# Use Node.js 18 as base image for the API server
FROM node:18
 
# Set working directory for the API application
WORKDIR /usr/api
...
# Expose port 3000 for the Node.js API server
EXPOSE 3000
...
```dockerfile
# Multi-stage build: Use Node.js 18 for building the React application
...
# Copy environment variables for React build
...
# Production stage: Use lightweight Alpine image with Node.js for serving
FROM node:18-alpine
WORKDIR /usr/src/app
# Install 'serve' package globally to serve static React build files
RUN npm install -g serve
# Copy the built React application from build stage
COPY --from=build /usr/src/app/build ./build
# Expose port 3001 for the React frontend
EXPOSE 3001
...
services:
  ...
  mongodb:
    ...
    ports:
      - "${MONGO_PORT:-27017}:27017"  # Map MongoDB port (default 27017)
    volumes:
      - mongo_data:${MONGO_DATA_PATH:-/data/db}  # Persist MongoDB data
    networks:
      - mail_network  # Connect to custom network

  # C++ TCP server for Bloom filter blacklist service  
  tcpserver:
    build:
      context: .  # Build context is project root
      dockerfile: backend/Dockerfile.tcpserver  # Dockerfile path
    container_name: tcp_backend
    ports:
      - "${TCP_PORT:-5555}:5555"  # Map TCP server port (default 5555)
    networks:
      - mail_network  # Connect to custom network
    command: ["./server", "${TCP_PORT:-5555}", "16", "1"]  # Override default command with env port

  # Node.js API server
  apiserver:
    build:
      context: .  # Build context is project root
      dockerfile: api/Dockerfile.api  # Dockerfile path
    container_name: api_server
    ports:
      - "${API_PORT:-3000}:3000"  # Map API server port (default 3000)
    depends_on:
      - tcpserver  # Wait for TCP server to start
      - mongodb    # Wait for MongoDB to start
    volumes:
      - ./api/uploads:/usr/api/uploads  # Mount uploads directory for file handling
    environment:
      # Environment variables for API server configuration
      TCP_SERVER_HOST: tcp_backend  # Hostname of TCP server within Docker network
      TCP_SERVER_PORT: ${TCP_PORT:-5555}  # Port of TCP server
      MONGO_URL: mongodb://mongodb_service:${MONGO_PORT:-27017}  # MongoDB connection string
      PORT: ${API_PORT:-3000}  # API server port
    networks:
      - mail_network  # Connect to custom network
    command: ["node", "server.js"]  # Start the Node.js server

  # React frontend web application
  frontend:
    build:
      context: .  # Build context is project root
      dockerfile: ./frontend/Dockerfile.react  # Dockerfile path
    container_name: frontend_app
    ports:
      - "${FRONTEND_PORT:-3001}:3001"  # Map frontend port (default 3001)
    depends_on:
      - apiserver  # Wait for API server to start
    environment:
      # API URL for frontend to communicate with backend
      REACT_APP_API_URL: http://localhost:${API_PORT:-3000}
    networks:
      - mail_network  # Connect to custom network
    command: ["serve", "-s", "build", "-l", "${FRONTEND_PORT:-3001}"]  # Serve React build with env port
